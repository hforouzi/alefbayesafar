...
use App\Modules\Hotel\Repository\HotelImageRepository;
use Doctrine\ORM\Mapping as ORM;
use Symfony\Component\Validator\Constraints as Assert;

#[ORM\Entity(repositoryClass: HotelImageRepository::class)]
#[ORM\HasLifecycleCallbacks]
#[ORM\Index(name: 'idx_hotel_image_hotel_primary', columns: ['hotel_id', 'is_primary'])]
#[ORM\UniqueConstraint(name: 'uniq_hotel_image_position', columns: ['hotel_id', 'position'])]
class HotelImage
{
    #[ORM\Id]
    #[ORM\GeneratedValue]
    ...
    {
        $this->hotel = $hotel;
        if ($this->primary && $hotel instanceof Hotel) {
            $hotel->markOnlyImagePrimary($this);
        }
    ...
    {
        if ($primary && $this->hotel instanceof Hotel) {
            $this->hotel->markOnlyImagePrimary($this);
    ...
    {
        if ($this->path === '') {
            throw new \LogicException('Hotel image path is required.');
        }
    }
}
